...
#define RUN_CLONE_RESTORE_FN(ret, clone_flags, new_sp, parent_tid,		\
	...
	do { \
		pr_msg("~Arm64~ Executing function: RUN_CLONE_RESTORE_FN in file: criu/arch/aarch64/include/asm/restorer.h\n"); \
		asm volatile(								\
				"clone_emul:					\n"	\
				"ldr x1, %2					\n"	\
				"and x1, x1, #~15				\n"	\
...
#define RUN_CLONE3_RESTORE_FN(ret, clone_args, size, args,			\
			      clone_restore_fn)					\
	do { \
		pr_msg("~Arm64~ Executing function: RUN_CLONE3_RESTORE_FN in file: criu/arch/aarch64/include/asm/restorer.h\n"); \
		asm volatile(								\
...
#define ARCH_FAIL_CORE_RESTORE					\
	do { \
		pr_msg("~Arm64~ Executing function: ARCH_FAIL_CORE_RESTORE in file: criu/arch/aarch64/include/asm/restorer.h\n"); \
		asm volatile(						\
				"mov sp, %0			\n"	\
				"mov x0, #0			\n"	\
				"b   x0				\n"	\
